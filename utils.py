...
#Face Blur
def resize_box(ori_img_size,boxes, img_size, margin=0):
    new_boxes=[]
    for box in boxes:
        box = [int(i) for i in box]
        margin_x = margin * (box[2] - box[0]) / (img_size - margin)
        margin_y = margin * (box[3] - box[1]) / (img_size - margin)
        box = [
            int(max(box[0]-margin_x/2,0)),
            int(max(box[1]-margin_y/2,0)),
            int(min(box[2]+margin_x/2, ori_img_size[1])),
            int(min(box[3]+margin_y/2, ori_img_size[0])),
        ]
